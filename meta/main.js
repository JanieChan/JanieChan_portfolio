...
function processCommits(data) {
  return d3
    ...
    .map(([commit, lines]) => {
      ...
      let ret = {
        id: commit,
        url: 'https://github.com/JanieChan/JanieChan_portfolio/commit/' + commit,
        author,
        date,
        time,
        timezone,
...
function renderCommitInfo(data, commits) {
    ...
    // Add longest file
    const longestFile = d3.rollups(
        data,
        (v) => d3.sum(v, (d) => d.line),
        (d) => d.file                       
    );
    const longestFileData = d3.max(longestFile, (d) => d[1]);
    const longestFileName = longestFile.find(f => f[1] === longestFileData)[0];
...
function onTimeSliderChange() {
    ...
    let commitTimeElement = document.getElementById("commit-time");
    commitTimeElement.textContent = commitMaxTime.toLocaleString('en-US', {dateStyle: "long", timeStyle: "short"});
